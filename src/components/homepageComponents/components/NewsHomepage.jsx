          ...
          <Slider {...settings}>
            {
              dataNewNews?.map((item, index) => {
                ...
                if(item.category==="Berita"){
                  Category = 
                    <div className="categoryCardsHomepage-title w-100 ci-bg-success" style={{padding: "2px 0", borderRadius: "4px"}}>
                      <p className="mb-0 font-11 font-weight-bold text-white text-center">{item.category}</p>
                    </div>
                }else if(item.category==="Pengumuman"){
                  Category = 
                    <div className="categoryCardsHomepage-title w-100 ci-bg-info" style={{padding: "2px 0", borderRadius: "4px"}}>
                      <p className="mb-0 font-11 font-weight-bold text-white text-center">{item.category}</p>
                    </div>
                }else if(item.category==="Artikel"){
                  Category = 
                    <div className="categoryCardsHomepage-title w-100 ci-bg-warning" style={{padding: "2px 0", borderRadius: "4px"}}>
                      <p className="mb-0 font-11 font-weight-bold text-dark text-center">{item.category}</p>
                    </div>
                }
                    ...
                    <Link to={`/berita/${item.slug}`} className="mt-1 mb-1 h-100">
                      <div className="card h-100 ci-customCard-1" style={{overflow: "hidden", borderRadius: "8px"}}>
                        <img
                          src={item.img_news}
                          alt="img_news"
                          style={{ width: "100%", overflow: "hidden", height: "150px"}}
                        />
                        <div className="card-body">
                          ...
                          <div className="row">
                            <div className="col-12">
                              <h5 className="font-15 text-gold" style={{minHeight: "80px"}}>{item.title}</h5>
                            </div>
                          </div>
                          <div className="row">
                            <div className="col-12 font-14 text-justify mt-2 text-homepage-p" style={{minHeight: "140px"}}>
                              {
