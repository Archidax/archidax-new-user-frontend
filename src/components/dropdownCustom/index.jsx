...
function DropdownCustom(props) {
    ...
    useEffect(() => {
        ...
        if(props.dataOptions){
            tmp = props.dataOptions.filter(val => String(val.text).toLowerCase().includes(search.toLowerCase()))
            setArr(tmp)
        }
    ...
    useEffect(() => {
        if(props.value !== undefined && props.value !==null) {
            props.dataOptions.forEach(value => {
                if(value.val === props.value){
                    setShow({val: value.val, text: value.text})
                }
            })
        }
    }, [props.value,props.dataOptions])
        ...
        <div className={`dropdown ci-dropdown ${props.classDropdown} font-12`}>
            ...
            <div className="ci-dropdown-menu dropdown-menu w-100" aria-labelledby="dropdownRiwayatTarikCrypto">
                ...
                <div className="container-fluid ci-dropdown-menu-content px-0 pb-0">
                    {arr && arr.map(option => {
                        return <button className="dropdown-item py-2 text-white font-12" type="button" onClick={() => {
                            props.onChange(option.val)
                        }} value={option.val}>{option.text}</button>
                    })}
                </div>
            </div>
        </div>
